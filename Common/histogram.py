...
def draw_hist_hue(hist, shape=(200, 256, 3)):
    ...
    hist_image = np.full(shape, 255, np.uint8)
    cv2.normalize(hist, hist, 0, shape[0], cv2.NORM_MINMAX)

    gap = hist_image.shape[1] / hist.shape[0]
    for i, h in enumerate(hist):
        x, w = int(round(i * gap)), int(round(gap))
        color = tuple(map(int, hsv_palette[i][0]))
        cv2.rectangle(hist_image, (x, 0, w, int(h)), color, cv2.FILLED)

    return cv2.flip(hist_image, 0)